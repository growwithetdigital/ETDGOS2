  ...
  const handleExportPDF = async (audit: AuditRecord) => {
    ...
    try {
      ...
      (audit.recommendations || [
        ...
        'Build out high-intent topical authority articles: monthly content plus tailored strategy calls'
      ]).forEach((r, i) => {
    ...
    <div className="space-y-8" id="auditor-archive-panel">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[var(--border)]">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display text-xl font-bold text-[var(--text)] tracking-tight">
                Growth Auditor & Diagnostics Engine
              </h3>
            </div>
            <p className="font-sans text-xs sm:text-sm text-[var(--muted)] mt-1 max-w-2xl leading-relaxed">
          ...
          {/* Mode Switcher */}
          <div className="flex items-center gap-1.5 p-1 bg-[var(--surface2)] rounded-xl border border-[var(--border)] text-xs font-display font-bold">
            <button
              type="button"
              onClick={() => setActiveAuditChannel('website')}
              className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer ${
                activeAuditChannel === 'website' ? 'bg-cyan-500 text-slate-950 shadow-sm font-bold' : 'text-[var(--muted)] hover:text-[var(--text)]'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer ${
                activeAuditChannel === 'social' ? 'bg-cyan-500 text-slate-950 shadow-sm font-bold' : 'text-[var(--muted)] hover:text-[var(--text)]'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer ${
                activeAuditChannel === 'gbp' ? 'bg-cyan-500 text-slate-950 shadow-sm font-bold' : 'text-[var(--muted)] hover:text-[var(--text)]'
              }`}
            >
        ...
        <form onSubmit={handleRunAudit} className="mt-6 space-y-4">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative flex-1">
              <input
                ...
                required
                className="w-full bg-[var(--surface2)] border border-[var(--border)] focus:border-cyan-500 rounded-2xl px-4 py-3.5 text-sm text-[var(--text)] placeholder:text-[var(--muted)] outline-none transition-colors pr-10 min-h-[46px]"
              />
            </div>
            ...
            <button
              ...
              className="px-6 py-3.5 rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-display text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md active:scale-95 disabled:opacity-50 shrink-0 min-h-[46px]"
            >
              {isAuditing ? (
                <>
          ...
          {isAuditing && (
            <div className="p-4 rounded-2xl bg-[var(--surface2)] border border-[var(--border)] space-y-2 animate-pulse">
              <div className="flex items-center justify-between text-xs font-mono text-cyan-600 font-bold">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-display text-lg font-bold text-[var(--text)] tracking-tight">
              Archived Audit Library ({archivedAudits.length})
            </h4>
          </div>
          <span className="text-xs text-[var(--muted)] font-mono">Permanent Historical Records</span>
        </div>

        {isLoading ? (
          <div className="py-12 text-center text-[var(--muted)] font-mono text-xs flex items-center justify-center gap-2">
          ...
          <div className="text-center py-12 px-4 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface2)] space-y-3">
            <div className="w-12 h-12 rounded-xl bg-[var(--surface)] flex items-center justify-center mx-auto text-cyan-500">
              <Globe className="w-6 h-6" />
            </div>
            <p className="font-display text-sm font-bold text-[var(--text)]">No Audits Archived Yet</p>
          ...
          <div className="space-y-4">
            {archivedAudits.map((audit) => {
              ...
              return (
                <div
                  key={audit.id}
                  className="rounded-2xl border border-[var(--border)] bg-[var(--surface2)] overflow-hidden transition-all"
                >
                  {/* Summary Bar */}
                  <div 
                    onClick={() => setExpandedAuditId(isExpanded ? null : (audit.id || null))}
                    className="p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 cursor-pointer hover:opacity-90 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-[10px] text-[var(--muted)] bg-[var(--surface)] px-2 py-0.5 rounded border border-[var(--border)]">
                    ...
                    <div className="flex items-center gap-3 self-end sm:self-auto">
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-xl bg-[var(--surface)] hover:opacity-80 text-[var(--text)] text-xs font-mono flex items-center gap-1.5 transition-colors cursor-pointer border border-[var(--border)]"
                      ...
                      <div className="text-[var(--muted)] p-1">
                  ...
                  {/* Expanded Breakdown */}
                  {isExpanded && (
                    <div className="px-4 sm:px-6 pb-6 pt-2 border-t border-[var(--border)] space-y-6">
                      {/* Metric Grid */}
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 pt-4">
                        <div className="p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-center">
                          <span className="font-mono text-[10px] text-[var(--muted)] block mb-1">Mobile Speed</span>
                          <span className="font-display text-base font-bold text-[var(--text)]">
                            {audit.metrics?.mobileSpeed || 72}/100
                          </span>
                        </div>
                        <div className="p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-center">
                          <span className="font-mono text-[10px] text-[var(--muted)] block mb-1">Desktop Speed</span>
                          <span className="font-display text-base font-bold text-[var(--text)]">
                            {audit.metrics?.desktopSpeed || 85}/100
                          </span>
                        </div>
                        <div className="p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-center">
                          <span className="font-mono text-[10px] text-[var(--muted)] block mb-1">SEO Health</span>
                          <span className="font-display text-base font-bold text-[var(--text)]">
                            {audit.metrics?.seoHealth || 80}/100
                          </span>
                        </div>
                        <div className="p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-center">
                          <span className="font-mono text-[10px] text-[var(--muted)] block mb-1">AEO / AI Search</span>
                          <span className="font-display text-base font-bold text-cyan-600">
                            {audit.metrics?.aeoScore || 65}/100
                          </span>
                        </div>
                        <div className="p-3 rounded-xl bg-[var(--surface)] border border-[var(--border)] text-center col-span-2 sm:col-span-1">
                          <span className="font-mono text-[10px] text-[var(--muted)] block mb-1">Conversion (CRO)</span>
                          <span className="font-display text-base font-bold text-[var(--text)]">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl bg-[var(--surface)] border border-[var(--border)] space-y-2">
                          <span className="font-display text-xs font-bold text-rose-500 uppercase tracking-wider flex items-center gap-1.5">
                            <AlertCircle className="w-3.5 h-3.5" />
                            <span>Identified Bottlenecks</span>
                          </span>
                          <ul className="space-y-1.5 text-xs text-[var(--text)] font-sans">
                            {(audit.growth_bottlenecks || [
                        ...
                        <div className="p-4 rounded-xl bg-[var(--surface)] border border-[var(--border)] space-y-2">
                          <span className="font-display text-xs font-bold text-cyan-600 uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        {onOpenBooking && (
                          <button
                            ...
                            className="text-xs font-display font-bold uppercase tracking-wider text-cyan-600 hover:underline flex items-center gap-1 cursor-pointer"
                          >
